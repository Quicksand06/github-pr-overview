...
pub fn draw(f: &mut Frame, app: &mut App) {
    ...
    let footer = Paragraph::new(Text::from(vec![Line::from(
        "Keys: a=add  d=delete  r=refresh  ↑/↓ or j/k=move  q=quit",
    )]))
    .alignment(Alignment::Center)
...
fn draw_repos_table(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    let table = Table::new(rows, [Constraint::Length(4), Constraint::Min(10)])
        ...
        .block(Block::default().borders(Borders::ALL).title("Monitored repos"))
...
fn draw_pr_overview_table(f: &mut Frame, app: &mut App, area: Rect) {
    ...
    .block(Block::default().borders(Borders::ALL).title("Open PRs overview"))
    .row_highlight_style(Style::default().add_modifier(Modifier::REVERSED));
...
fn join_truncated(items: &[String], max: usize, sep: &str) -> String {
    ...
    let head = items.iter().take(max).cloned().collect::<Vec<_>>().join(sep);
